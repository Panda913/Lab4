 ...
 * @author Привет
 */
@WebServlet(name = "addNewFacultyServlet", urlPatterns = {"/addNewFacultyServlet"})
public class addNewFacultyServlet extends HttpServlet {
    ...
    private HttpServletRequest request;
      ...
      protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if( NameFaculty.isEmpty()){ RequestDispatcher rd  = request.getRequestDispatcher("Error.jsp");
       rd.forward(request, response);} else {
        Faculty faculty = new Faculty(NameFaculty,new ArrayList());
       
        
        Institute.addFaculty(faculty);
        request.getSession().setAttribute("result",Institute.getFaculties() );}
